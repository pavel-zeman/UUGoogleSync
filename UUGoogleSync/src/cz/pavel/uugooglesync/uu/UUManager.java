	...
	public Map<String, UUEvent> getEvents(Calendar startDate, Calendar endDate) throws ClientProtocolException, IOException {
        ...
        while (!currentDate.after(endDate)) {
	        ...
	        while ((nextIndex = data.indexOf("<DIV class=\" normal-diary-item-long", nextIndex)) >= 0) {
	        	...
	        	String id = HtmlParser.extractRegExp(itemData, "<SPAN [^>]*id=\"dwdiary_item_([0-9]*)_[0-9]*-");
	        	String summary = HtmlParser.extractRegExp(itemData, "<SPAN [^>]*id=\"dwdiary_item_[^>]*>([^<]*)</SPAN>", true);
	        	String place = HtmlParser.extractRegExp(itemData, "<SPAN [^>]*id=\"dwdiary_item_[^>]*>[^<]*</SPAN>, (.*)</SPAN></DIV>", true);
