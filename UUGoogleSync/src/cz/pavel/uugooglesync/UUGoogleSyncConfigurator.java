...
public class UUGoogleSyncConfigurator extends JFrame implements ActionListener {
	...
	public UUGoogleSyncConfigurator() {
		...
		this.add(getLabel("Poèet dnù pro synchronizaci pøed a po aktuálním dnu"), getConstraints(0, 1));
		...
		this.add(getLabel("V prohlížeèi pøejdìte na následující URL a postupujte podle instrukcí (staèí pøi prvním použití)"), getConstraints(0, 5, true, 3));
		...
		this.add(getLabel("Kód vygenerovaný pomocí pøedchozího URL"), getConstraints(0, 7));
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (googleCode.getText().length() > 0) {
			...
			try {
				...
				JOptionPane.showMessageDialog(this, "Chyba pøi generování Google tokenù", "Chyba", JOptionPane.ERROR_MESSAGE);
				return;
			}
		}
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "Chyba pøi aktualizaci konfigurace", "Chyba", JOptionPane.ERROR_MESSAGE);
			return;
		}
